...
def correctness_or_hinted_code(prompts, completions, evaluator, answer, gt_answer, func_name, setup_code, **kwargs) -> list[float]:
    ...
    wandb_log({
        ...
        'detail/avg_hinted_pass_rate': sum([x['pass_rate'] for x in code_eval_detailed]) / len(code_eval_detailed),
        'detail/avg_n_hinted_tests': sum([x['tests_total'] for x in code_eval_detailed]) / len(code_eval_detailed),
