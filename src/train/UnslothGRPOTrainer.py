...
def run_with_cache_activations(model, tokenizer, prompts, completions, layers, position):
...
class _UnslothGRPOTrainer(Trainer):
    ...
    def _generate_and_score_completions(
        ...
        if self.cache_activations:
            with torch.inference_mode():
                with unwrap_model_for_generation(
                    ...
                    activations = run_with_cache_activations(
        ...
        # FIXME: APPLY SCREENING OF GRPO EXAMPLES HERE
        # Need to modify the group_sizes calculation to accept reducing the number of examples at this stage
        # Need to adjust the total number of generations accordingly as well
        # Alternatively set those advantages to -3? Not clear

        # Emit a single log message the first time the patched trainer runs so we can verify the replacement was loaded.
        if not hasattr(self, "_unsloth_patch_logged"):
            self.accelerator.print(
                "[unsloth-grpo] Patched advantage computation active "
            ...
            self._unsloth_patch_logged = True

        # Derive how many completions we actually produced per prompt. The repeat sampler ensures identical prompts
        # appear consecutively, so we can recover the group sizes from the prompt ids even when multiple ranks are
        # involved. We gather the per-rank counts to stay in sync with the gathered rewards above.
        group_sizes = torch.unique_consecutive(prompt_ids, dim=0, return_counts=True)[1]
        group_sizes = gather(group_sizes.to(device))

        total_generations = rewards.numel()
        if group_sizes.numel() == 0:
            raise RuntimeError("No prompts found in the generation batch; cannot compute GRPO advantages.")
        if group_sizes.sum().item() != total_generations:
            raise RuntimeError(
                f"Mismatch between reward count ({total_generations}) and grouped prompt counts "
                f"({group_sizes.sum().item()}). Group sizes: {group_sizes.cpu().tolist()}."
            )
        if not torch.all(group_sizes == self.num_generations):
            self.accelerator.print(
                f"[warning] Expected {self.num_generations} completions per prompt but observed "
                f"{group_sizes.cpu().tolist()} for this batch "
                f"(local_batch={len(prompts)}, total_generations={total_generations}, "
                f"per_device_train_batch_size={self.args.per_device_train_batch_size}, "
                f"steps_per_generation={self.args.steps_per_generation})."
            )
        ...
        repeat_pattern = group_sizes.to(device)
        mean_grouped_rewards = mean_grouped_rewards.repeat_interleave(repeat_pattern, dim=0)
        std_grouped_rewards = std_grouped_rewards.repeat_interleave(repeat_pattern, dim=0)
